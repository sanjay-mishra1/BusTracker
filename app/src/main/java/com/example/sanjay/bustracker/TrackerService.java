...
package com.example.sanjay.bustracker;
...
public class TrackerService extends Service implements LocationListener {
    ...
    private LinkedList<Map<String, Object>> mTransportStatuses = new LinkedList<Map<String, Object>>();
    ...
    public void onCreate() {
        ...
        mPrefs = getSharedPreferences(getString(R.string.prefs), MODE_PRIVATE);
          email = mPrefs.getString(getString(R.string.email), "");
          route = mPrefs.getString(getString(R.string.route), "");
    ...
    private void authenticate(final String email, final String password) {
        ...
        mAuth.createUserWithEmailAndPassword(email+"@gmail.com", password)
            ...
            public void onFailure(@NonNull Exception e) {
                Log.e(TAG, "authenticate:via sign in  started");
                ...
                if(e.getMessage().contains("already in use")){
                    mAuth.signInWithEmailAndPassword(email+"@gmail.com", password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            Log.e(TAG, "authenticate:via sign in " + task.isSuccessful());
                            if (task.isSuccessful()) {
                                fetchRemoteConfig();
                                loadPreviousStatuses();
                            } else {
    ...
    private void loadPreviousStatuses() {
          ...
          transportId = mPrefs.getString(getString(R.string.transport_id), null);
        iconPath= getString(R.string.imageLinkFirstPart)+"bus%20("+transportId+").png?alt=media&token=5589e904-66e8-4d17-80b8-e1c18977ef2b";
            ...
            public void onDataChange(DataSnapshot snapshot) {
                if (snapshot != null) {
                    for (DataSnapshot transportStatus : snapshot.getChildren()) {
                      try {
                          if(Objects.equals(transportStatus.getKey(), "a"))
                          {  mTransportStatuses.add(0,
                                  (Map<String, Object>) transportStatus.getValue());
                          }
                          else mTransportStatuses.add(1,
                                  (Map<String, Object>) transportStatus.getValue());
                      }catch (Exception e){
                          e.printStackTrace();
                      }
...
/*
    ...
    public void onLocationChanged(Location location) {
       ...
       // transportStatus.put("iconPath",getString(R.string.imageLinkFirstPart)+"bus%20("+transportId+"?alt=media&token=5589e904-66e8-4d17-80b8-e1c18977ef2b");
       ...
       // transportStatus.put("power", getBatteryLevel());
        ...
        if (locationIsAtStatus(location, 1) && locationIsAtStatus(location, 0)) {
            ...
            //mFirebaseTransportRef.child("a").setValue(transportStatus);
            store(transportStatus);
             Log.e(TAG,"creating only a");
        } else {
           ...
           store(mTransportStatuses.get(0));
    ...
    private void store(final Map<String,Object> stringObjectMap) {
        mFirebaseTransportRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                mFirebaseTransportRef.child("iconPath").setValue(iconPath);
                mFirebaseTransportRef.child("name").setValue(transportId);
                mFirebaseTransportRef.child("mobile").setValue(email);
                mFirebaseTransportRef.child("route").setValue(route);

                     Log.e("store loop",""+dataSnapshot.getChildren());
                    if (!dataSnapshot.hasChild("a")||!dataSnapshot.exists()) {

                        mFirebaseTransportRef.child("a").setValue(stringObjectMap);
                    } else if (!dataSnapshot.hasChild("b")) {
                        Log.e("store", "b not exist");
                        mFirebaseTransportRef.child("b").setValue(stringObjectMap);
                    } else {
                        Log.e("store", "both exist");
                        if ((long) dataSnapshot.child("a").child("time").getValue() > (long) dataSnapshot.child("b").child("time").getValue()) {
                            Log.e("store", "both exist writing in b");
                            mFirebaseTransportRef.child("b").setValue(stringObjectMap);
                        } else {
                            Log.e("store", "both exist writing in a");
                            mFirebaseTransportRef.child("a").setValue(stringObjectMap);
                        }
    ...
    private synchronized String createChannel() {
        ...
        NotificationChannel mChannel = new NotificationChannel("snap map channel", getString(R.string.app_name), importance);
    ...
    private void buildNotification() {
        ...
        PendingIntent resultPendingIntent = PendingIntent.getActivity(this, 0,
                new Intent(this, TrackerActivity.class), PendingIntent.FLAG_UPDATE_CURRENT);
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O){
            String NOTIFICATION_CHANNEL_ID = "com.example.BusTracker";
            String channelName = "Tracker Background Service";
